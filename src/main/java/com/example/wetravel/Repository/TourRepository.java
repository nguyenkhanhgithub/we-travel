...
public interface TourRepository extends JpaRepository<Tour , Long> {
    @Query(value = "SELECT * FROM tour where (tour_name like :tourName or \"all\" like :tourName) and (tour_category_id in :tourCategoryList or -1 in :tourCategoryList)" +
            " and (tour_type = :tourType or -1 = :tourType) and (status = :status or -1 = :status) and ((adult_price between :fromPrice and :toPrice) or (-1 = :fromPrice and -1 = :toPrice)) order by tour_id desc" , nativeQuery = true)
