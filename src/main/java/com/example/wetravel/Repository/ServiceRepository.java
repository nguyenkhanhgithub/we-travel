...
public interface ServiceRepository extends JpaRepository<Service, Long> {
    ...
    @Query(value = "select s.* from service s join partner p on s.partner_id = p.partner_id join \n" +
            ...
            " and (s.status = :status or -1 = :status) and (s.service_id in (:serviceIdList) or -1 in (:serviceIdList)) order by s.service_id desc " , nativeQuery = true)
    ...
    @Query(value = "select s from Service s join Partner p on s.partnerId.partnerId = p.partnerId join \n" +
            "Account a on p.accountId.accountId = a.accountId where p.partnerId = :partnerId order by s.serviceId desc ")
